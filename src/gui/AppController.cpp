...
bool AppController::setParameter(const QString &name, const QVariant &value)
{
    ...
    bool success = m_driver->setParameter(name, value);
    if (success) {
...
bool AppController::setParameters(const QHash<QString, QVariant> &params)
{
    ...
    if (!canConfigure()) {
        qWarning() << "AppController: Cannot configure during acquisition";
        return false;
    }
    ...
    for (auto it = params.begin(); it != params.end(); ++it) {
        if (!m_driver->setParameter(it.key(), it.value())) {
            success = false;
        } else {
                                       ...
                                       const QString &cameraId)
{
    ...
    if (m_state != CameraState::Acquiring && m_state != CameraState::Connected) {
        return;
    }
    ...
    frame.parameters = QVariantMap();
    for (auto it = m_parameters.begin(); it != m_parameters.end(); ++it) {
        frame.parameters[it.key()] = it.value();
    }
...
{
    Q_UNUSED(cameraId);
    if (!connected && m_state != CameraState::Disconnected) {
        qWarning() << "AppController: Driver reports disconnection";
        disconnectFromDriver();
        enterErrorState(CameraError::makeError(
            CameraError::Code::NotConnected,
...
{
    qWarning() << "AppController: Driver error:" << static_cast<int>(error.code)
               << error.description;

    bool recoverable = (error.code == CameraError::Code::InvalidParameter ||
                        error.code == CameraError::Code::ValueOutOfRange ||
                        error.code == CameraError::Code::Timeout ||
                        error.code == CameraError::Code::StateInvalid);
...
void AppController::setState(CameraState newState)
{
    ...
    qDebug() << "AppController: State" << static_cast<int>(oldState)
